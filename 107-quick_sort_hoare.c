...
int hoare_partition(int *array, size_t size, int left, int right)
{
	...
	/**
	 * Choose the last element as pivot and loop through
	 * the partition to order its elements.
	 */
	pivot = array[right];
	for (above = left - 1, below = right + 1; above < below;)
	{
		...
		/* Find the next element smaller than the pivot */
		do {
			below--;
		} while (array[below] > pivot);

		/* Swap the two elements if above is less than below */
		if (above < below)
		{
			swap(array + above, array + below, sizeof(int));
...
void hoare_sort(int *array, size_t size, int left, int right)
{
	...
	/* If the partition has at least two elements */
	if (right - left > 0)
	{
		/* Order the partition and its two halves recursively */
